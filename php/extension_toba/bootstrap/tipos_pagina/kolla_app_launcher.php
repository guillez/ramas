...
class kolla_app_launcher
{
    ...
    static function instancia($recargar = false)
    {
        if (!isset(self::$instancia) || $recargar) {
            self::$instancia = new kolla_app_launcher($recargar);
        }
    ...
    private function cargar_app_launcher_data()
    {
        ...
        $appLauncherDataDefault = array(
            'kolla_usuario_id' => toba::usuario()->get_id(),
        ...
        //recupero los ids de esas cuentas
        $cuentas = toba::manejador_sesiones()->get_autenticacion()->get_lista_cuentas_posibles();
        $cuentas_con_datos = toba_usuario_basico::recuperar_descripcion_cuentas($cuentas);

        // Asumo el orden de ambos arreglos es el mismo, entonces reemplazo el nombre y apellido de uno
        // por la cuenta que tengo en el otro.
        $cant = count($cuentas_disponibles) - 1; // saco uno porque esta la cuenta actual como último valor
        for ($i = 0; $i < $cant; $i++) {
            $cuentas_disponibles[$i]['descripcion'] = $cuentas_con_datos[$i]['id'];
        }
    ...
    public function get_html_app_launcher()
    {
        ...
        $url_base_2 = toba_recurso::url_proyecto() . '/js/';
        ...
        $html .= toba_js::incluir($url_base_2 . 'kolla_app_launcher.js');
        $html .= '<div id="enc-usuario" class="enc-usuario arai-fix-div"></div>';
