...
class form_persona extends bootstrap_formulario
{
	...
	function extender_objeto_js()
	{
		...
		{$this->objeto_js}.evt__fecha_nac__validar = function()
		{
			if (this.ef('fecha_nac').tiene_estado()) {
				if (!this.valida_fecha()) {
					this.ef('fecha_nac').set_error('$mensaje_fecha_nac');
					return false;
				}
			}
		...
		{
			var fecha = this.ef('fecha_nac').get_estado();
