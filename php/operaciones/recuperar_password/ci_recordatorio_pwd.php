...
class ci_recordatorio_pwd extends bootstrap_ci
{
	...
	function conf__form_usuario(toba_ei_formulario $form)
	{
		...
		$form->set_descripcion('Se enviará un mail a su casilla de correo electrónico con instrucciones de cómo restablecer su contraseña.');
	}
	...
	function evt__form_usuario__modificacion($datos)
	{
		if ( $datos['captcha'] ) {
			...
			throw new toba_error_autenticacion('El código de verificación ingresado no es válido');
		}
	}
	...
	function conf__pant_inicial(toba_ei_pantalla $pantalla)
	{
		...
		if (isset($this->randr) && ! is_null($this->randr)) {
			...
			$pantalla->set_descripcion('La nueva contraseña fue enviada a su cuenta de mail.');
		}
	}
	...
	function enviar_mail_aviso_cambio()
	{
		...
		$cuerpo_mail = '<p>Este mail fue enviado a esta cuenta porque se <strong>solicito un cambio de contraseña</strong>.</p>'
		. '<p>Si usted solicito dicho cambio haga click en el siguiente enlace:</p>'
        . '<a href=' . $link . '>' . $link . '</a>'
		. '<p>El mismo será válido unicamente por 24hs.</p>';
	...
	function disparar_confirmacion_cambio()
	{
		...
		$cuerpo_mail = '<p>Se ha recibido su confirmación exitosamente, su contraseña fue cambiada a:</p>' .
		$clave_tmp . '<p>Por favor en cuanto pueda cambiela a una contraseña más segura.</p> <p>Gracias.</p> ';
