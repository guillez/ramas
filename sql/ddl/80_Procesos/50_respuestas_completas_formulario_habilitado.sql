-- Function: respuestas_completas_formulario_habilitado(integer)

-- DROP FUNCTION respuestas_completas_formulario_habilitado(integer);

CREATE OR REPLACE FUNCTION respuestas_completas_formulario_habilitado(integer)
...
WHERE srf.formulario_habilitado = form AND (sp.tabla_asociada = '' OR sp.tabla_asociada IS NULL) ;
...
FOR tabla IN 
  ...
  LOOP
	...
	FROM sge_respondido_formulario srf 
	  ...
	  LEFT JOIN sge_elemento selem ON (selem.elemento = sfhd.elemento)  
      LEFT JOIN sge_concepto sconc ON (sconc.concepto = sfh.concepto) 
	WHERE srf.formulario_habilitado = ' || form || ' 
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION respuestas_completas_formulario_habilitado(integer)
  OWNER TO postgres;
